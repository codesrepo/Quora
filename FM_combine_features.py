...
train_cosine = pd.read_pickle(path+'train_cosine.pkl')[:].reshape(-1,1)
test_cosine = pd.read_pickle(path+'test_cosine.pkl')[:].reshape(-1,1)

train_cosine[np.isnan(train_cosine)] = -10
test_cosine[np.isnan(test_cosine)] = -10
...
skf = StratifiedKFold(n_folds=3,y=y, shuffle=False, random_state=seed)
