...
def main():
    ...
    path='C:/Quora/' # set your input folder here
    ...
    test["question1"] = test["question1"].apply(lambda x:text_to_wordlist(x))
    test["question2"] = test["question2"].apply(lambda x:text_to_wordlist(x))
    
    print('Generate cosine similarity')
    train_cosine = train.apply(parse_sent_word2vec, axis=1, raw=True)
    test_cosine = test.apply(parse_sent_word2vec, axis=1, raw=True)
    pd.to_pickle(train_cosine,path+"train_cosine.pkl")
    pd.to_pickle(test_cosine,path+"test_cosine.pkl")
