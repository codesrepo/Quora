...
if __name__ == '__main__':
    ...
    for metric in param_fairness['protected_attribute_list']:     
        ...
        path_temp = param_fairness["out_path"]+"%s_initial_fairness_report.csv"%(metric)
        print("..saving...%s_initial_fairness_report.csv"%(metric))
    ...
    for metric in metric_summary_paths:
        path_temp = param_fairness["out_path"]+"%s_initial_fairness_report.csv"%(metric)
        save_path = param_fairness["out_path"]+"%s_initial_fairness_plot.png"%(metric)
        df_metrices = pd.read_csv(path_temp)
        df_metrices=df_metrices[df_metrices.threshold>0.85]
        line_plot(df_metrices,'Score','Fairness values','Fairness report- Base model (%s)'%(metric),['threshold'],["threshold"]+param_fairness['track_metrices'],'threshold','value',vertical=0.92,save_path=save_path)
